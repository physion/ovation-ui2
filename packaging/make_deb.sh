...
fpm --force -s dir -t deb -n ovation -v $VERSION --epoch $BUILD --license gpl3 --vendor Physion --category Science -d 'couchdb >= 1.4' -d 'java7-runtime' --url http://ovation.io -m 'Physion <support@ovation.io>' --description "$DESCRIPTION"  $TARGET_DIR=/opt $ICON_DIR=/usr/share/icons/hicolor/48x48 $DESKTOP_INFO_DIR=/usr/share
